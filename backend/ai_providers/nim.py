...
class NVIDIANIMProvider(AIProvider):
    ...
    async def enhance_recommendation(
        ...
        try:
            ...
            return {
                "enhanced_description": result.get("description", f"A stylish {outfit.occasion} outfit."),
        ...
        try:
            ...
            response = await client.post(
                ...
                json={
                    "model": self.model,
                    "messages": [
                        {"role": "system", "content": "You are a fashion stylist. Generate a concise, appealing outfit description in one paragraph."},
                        {"role": "user", "content": prompt}
                    ],
                    "temperature": 0.7,
                    "max_tokens": 200
...
Enhance this recommendation with an engaging description and practical styling tips."""
    ...
    def _build_description_prompt(
        ...
        garment_desc = ", ".join([
            f"{g.color_name} {g.type} ({g.pattern})" for g in garments
        ])
