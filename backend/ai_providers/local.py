...
class LocalRulesProvider(AIProvider):
    ...
    async def enhance_recommendation(
        ...
        if garments:
            ...
            if len(garments) == 1:
                item = garments[0]
                description_parts.append(f"A {item.color_name} {item.type} perfect for {outfit.occasion}.")
        ...
        return {
            "enhanced_description": " ".join(description_parts) if description_parts else f"A stylish {outfit.occasion} outfit.",
