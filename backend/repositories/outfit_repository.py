...
class OutfitRepository:
    ...
    def create(self, outfit: OutfitCreate) -> Outfit:
        ...
        for position, garment_id in enumerate(outfit.garment_ids):
            link = OutfitGarmentLink(
                outfit_id=db_outfit.id, garment_id=garment_id, position=position
            )
    ...
    def update(self, outfit_id: int, outfit: OutfitUpdate) -> Outfit | None:
    ...
    def get_garments_for_outfit(self, outfit_id: int) -> list[int]:
        ...
        statement = (
            select(OutfitGarmentLink)
            .where(OutfitGarmentLink.outfit_id == outfit_id)
            .order_by(OutfitGarmentLink.position)
        )
    ...
    def add_garment_to_outfit(self, outfit_id: int, garment_id: int, position: int = None) -> bool:
        ...
        existing = self.session.exec(
            select(OutfitGarmentLink).where(
                OutfitGarmentLink.outfit_id == outfit_id, OutfitGarmentLink.garment_id == garment_id
            )
        ).first()
        if existing:
            return False
