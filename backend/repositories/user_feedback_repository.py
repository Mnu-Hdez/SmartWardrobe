...
class UserFeedbackRepository:
    ...
    def get_all(self, skip: int = 0, limit: int = 100) -> List[UserFeedback]:
        statement = select(UserFeedback).offset(skip).limit(limit).order_by(UserFeedback.created_at.desc())
    ...
    def get_recent(self, days: int = 30, limit: int = 100) -> List[UserFeedback]:
        ...
        statement = select(UserFeedback).where(UserFeedback.created_at >= cutoff).order_by(UserFeedback.created_at.desc()).limit(limit)
        return list(self.session.exec(statement).all())
