...
from backend.repositories import GarmentRepository, OutfitRepository, StyleRuleRepository, UserFeedbackRepository
from backend.models.garment import Garment, Outfit, StyleRule, UserFeedback
...
from backend.services import StyleEngine, OutfitComposer, FeedbackService, PackingService
...
async def create_garment(
    ...
    """Create a new garment with image upload and AI analysis."""
    repo = GarmentRepository(session)
    settings = get_settings()
    
    # Save original image
    original_dir = Path(settings.images_original_dir)
    ...
    mask, masked_img, seg_conf = segmenter.segment_auto(Image.open(original_path))
    ...
    classification = classifier.classify(Image.open(original_path))
...
async def list_outfits(
    ...
    season: Optional[str] = Query(None),
    is_packing: Optional[bool] = Query(None),
    session: Session = Depends(get_session)
):
...
async def get_garment_feedback(
    garment_id: int,
    session: Session = Depends(get_session)
):
