...
class IngestionPipeline:
    ...
    def process_garment(
        ...
        original_dir = Path(self.settings.images_original_dir)
        ...
        import shutil
        stored_original = original_dir / base_name
        shutil.copy2(image_path, stored_original)
        ...
        if self.classifier:
            try:
                ...
                print(f"Classification: {classification['type']} ({classification['type_confidence']:.2f})")
            except Exception as e:
                print(f"Classification failed: {e}")
        
        # 3. Color extraction
        color_info = {"dominant_color_hex": "#000000", "dominant_color_name": "black", "palette": []}
        
        try:
            color_info = extract_colors_from_image(
                masked_image_path or str(stored_original),
                mask_path
            )
            print(f"Dominant color: {color_info['dominant_color_name']} ({color_info['dominant_color_hex']})")
        except Exception as e:
            print(f"Color extraction failed: {e}")
        
        # 4. Create garment record
        garment_data = GarmentCreate(
            ...
            notes=notes,
            image_path=str(stored_original),
        ...
        return {
            ...
            "original_image": str(stored_original),
            "processed_image": masked_image_path,
            "mask_image": mask_path,
            "confidence_scores": json.loads(garment.confidence_scores) if garment.confidence_scores else {},
